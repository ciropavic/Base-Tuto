...
CreateThread(function()
	while true do
		...
		if ESX.GetPlayerData().job2 and ESX.GetPlayerData().job2.name == 'ambulance' then
			local playerCoords = GetEntityCoords(PlayerPedId())
			local isInMarker, hasExited = false, false
			local currentHospital, currentPart, currentPartNum
...
RegisterCommand("ambulance", function(src)
	if (ESX.GetPlayerData().job or ESX.GetPlayerData().job2) and (ESX.GetPlayerData().job.name == 'ambulance' or ESX.GetPlayerData().job2.name == 'ambulance') and not ESX.GetPlayerData().dead then 
		OpenMobileAmbulanceActionsMenu()
	end
end)

RegisterKeyMapping("ambulance", "Menu Ambulancier", "keyboard", "F6")
