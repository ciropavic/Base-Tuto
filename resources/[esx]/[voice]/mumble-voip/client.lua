...
AddEventHandler("onClientResourceStart", function(resName) -- Initialises the script, sets up voice range, voice targets and request sync with server
	...
	CheckVoiceSetting("profile_voiceEnable", "Chat vocal désactivé")
	CheckVoiceSetting("profile_voiceTalkEnabled", "Microphone désactivé")

	if not MumbleIsConnected() then
		SendNUIMessage({ warningId = "mumble_is_connected", warningMsg = "Non connecté au chat vocal" })
